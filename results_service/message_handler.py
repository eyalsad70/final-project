...
def send_restaurant_list(json_message, chat_id):
    ...
    for idx, place in enumerate(places):    
        ...
        test_message = f"{idx+1}: Name={name} ; Address={address} ; Opening-hours={opening_hours} ; Rating={rating} ; PriceLevel={price_level} \n \
            web-site {website} \n Google-Maps {google_url} "
        if not send_message(chat_id,test_message):
            return False        
    return False
...
def send_gas_stations_list(json_message, chat_id):
    ...
    for idx, place in enumerate(places):    
        ...
        test_message = f"{idx+1}: Name={name} ; Address={address} ; Opening-hours={opening_hours} Services=({services_str}) \n  {google_url} "
        if not send_message(chat_id,test_message):
            return False        
    return True
